...
class Game extends JPanel {
    ...
    private static Font FonT_M2 = new Font("MV Boli",Font.PLAIN,50);
    ...
    private void update() {
        ...
        if (apple != null && snake.getHead().intersects(apple, 20)) {
            ...
            apple = null;
            points++;
            changeAppleColor(); // Change the color of the apple
        }
    ...
    private void render(Graphics g) {
        ...
        if (status == GameStatus.NOT_STARTED) {
            ...
            drawCenteredString(g2d,"Developed by Mina Robir",FonT_M2,430);
            return;
        }
        ...
        if (apple != null) {
            g2d.setColor(appleColor); // set the color of the apple
            g2d.fillOval(apple.getX(), apple.getY(), 10, 10);
        }

        if (status == GameStatus.GAME_OVER) {
            g2d.setColor(Color.WHITE);
            drawCenteredString(g2d,"Try Again",FonT_M2,350);
    ...
    private void changeAppleColor() {
        Random random = new Random();
        appleColor = new Color(random.nextInt(256), random.nextInt(256), random.nextInt(256));
    }
...
class Point {
    ...
    public boolean intersects(Point p, int num) {
        int diffX = Math.abs(x - p.getX());
        int diffY = Math.abs(y - p.getY());

        return this.equals(p) || (diffX <= num && diffY <= num);
    }
}
